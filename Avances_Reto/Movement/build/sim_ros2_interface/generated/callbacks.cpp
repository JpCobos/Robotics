...
void write__tf2_msgs__msg__TFMessage(const tf2_msgs::msg::TFMessage& msg, int stack, const ROS2WriteOptions *opt)
{
    try
    {
        sim::pushTableOntoStack(stack);
        try
        {
            ...
            sim::pushStringOntoStack(stack, "transforms", 0);
            sim::pushTableOntoStack(stack);
            for(int i = 0; i < msg.transforms.size(); i++)
            {
                write__int32(i + 1, stack, opt);
                write__geometry_msgs__msg__TransformStamped(msg.transforms[i], stack, opt);
                sim::insertDataIntoStackTable(stack);
            }
            sim::insertDataIntoStackTable(stack);
        }
    ...
    {
        std::string msg = "write__tf2_msgs__msg__TFMessage: ";
        msg += ex.what();
        throw sim::exception(msg);
    }
}

void read__tf2_msgs__msg__TFMessage(int stack, tf2_msgs::msg::TFMessage *msg, const ROS2ReadOptions *opt)
{
    try
    {
        int r = sim::getStackTableInfo(stack, 0);
                ...
                else if(strcmp(str, "transforms") == 0)
                {
                    try
                    {
                        ...
                        for(int i = 0; i < numItems1; i++)
                        {
                            ...
                            read__int32(stack, &j, opt);
                            sim::moveStackItemToTop(stack, oldsz1 - 1); // move value to top
                            geometry_msgs::msg::TransformStamped v;
                            read__geometry_msgs__msg__TransformStamped(stack, &v, opt);
    ...
    {
        std::string msg = "read__tf2_msgs__msg__TFMessage: ";
        msg += ex.what();
        throw sim::exception(msg);
    }
}
...
void ros_callback__tf2_msgs__msg__TFMessage(const tf2_msgs::msg::TFMessage::SharedPtr msg, SubscriptionProxy *proxy)
{
    int stack = -1;
    try
    {
        stack = sim::createStack();
        write__tf2_msgs__msg__TFMessage(*msg, stack, &(proxy->wr_opt));
        sim::callScriptFunctionEx(proxy->topicCallback.scriptId, proxy->topicCallback.name.c_str(), stack);
        sim::releaseStack(stack);
        stack = -1;
    }
    catch(std::exception &ex)
    {
        if(stack != -1)
            sim::releaseStack(stack); // don't throw
        std::string msg = "ros_callback__tf2_msgs__msg__TFMessage: ";
        msg += proxy->topicCallback.name + ": ";
        msg += ex.what();
        sim::setLastError(msg);
    }
